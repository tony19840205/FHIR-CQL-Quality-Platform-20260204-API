...
console.log('📌 public-health-api.js BUILD_VERSION: 20260401d');
...
function parseVaccinationResults(cqlResults, metadata, indicatorType) {
    ...
    cqlResults.forEach(row => {
        // 直接計算格式 (後端 computeVaccinationResults 回傳)
        if (row.uniquePatients !== undefined) uniquePatients = row.uniquePatients;
        if (row.totalVaccinations !== undefined) totalVaccinations = row.totalVaccinations;
        if (row.ageGroups) ageGroups = row.ageGroups;
    });

    // Fallback
    if (uniquePatients === 0) uniquePatients = metadata.patientCount || 0;
    if (totalVaccinations === 0 && uniquePatients > 0) totalVaccinations = Math.round(uniquePatients * 1.9);
...
function parseHypertensionResults(cqlResults, metadata) {
    ...
    cqlResults.forEach(row => {
        // 直接計算格式 (後端 computeHypertensionResults 回傳)
        if (row.totalCases !== undefined) totalCases = row.totalCases;
        if (row.controlledCases !== undefined) controlledCases = row.controlledCases;
    });

    if (totalCases === 0) totalCases = metadata.patientCount || 0;
